...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    return restaurante || "No se ha encontrado ningún restaurant";
}
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    var ciudadesSinRepetir = eliminarElementosRepetidos( ciudades )
...
Listado.prototype.obtenerRubros = function() {
    ...
    var rubrosSinRepetir = eliminarElementosRepetidos( rubros )
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var horarios = this.restaurantes.map( restaurante => restaurante.horarios )

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var todosLosHorarios = [];
    horarios.forEach(function(arrayDeHorarios) {
        arrayDeHorarios.forEach(function(hor) {
            todosLosHorarios.push(hor)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosSinRepetir = eliminarElementosRepetidos( todosLosHorarios )
...
const eliminarElementosRepetidos = ( array ) => array.filter( (elem, index, self)  => index === self.indexOf(elem) )
